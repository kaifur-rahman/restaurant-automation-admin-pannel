...
const newColumns=data.addDatesFreeSubs();
//filtering the data from rows to get all those rows who's status is approved
const filterRows=data.rows.filter(item=>item.status.includes("Free Subscription"))

function freeSubscription(){
  return(
    <div>
    <Title title="Free Subscription"/>
    <br/>
    <TaskBar tools={<FullScreen title="Free Subscription" tableRows={filterRows} tableColumns={newColumns} height={'100vh'} rowsPerPage={10}/>}/>
