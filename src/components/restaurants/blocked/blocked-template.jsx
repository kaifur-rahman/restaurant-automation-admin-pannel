...
const newColumns=data.addBlockedDate();
//filtering the data from rows to get all those rows who's status is approved
const filterRows=data.rows.filter(item=>item.status.includes("Blocked"))
...
function blocked(){
  return(
    ...
    <Title title="Blocked"/>
    <br/>
    <TaskBar tools={<FullScreen title="Blocked" tableRows={filterRows} tableColumns={newColumns} height={'100vh'} rowsPerPage={10}/>}/>
